...
;; Simply 10.1
(define (already-won? position l)
  (not (empty? (filter (lambda (triple)
			 (equal? triple (word l l l)))
		       (find-triples position)))))

;; Simply 10.2
(define (tie-game? position)
  (= (number-of-free-squares position) 0))

;; Simply 10.3
(define (tie-game? position)
  (and (last-move? position)
       (not (i-can-win? (find-triples position) 'x))))

(define (last-move? position)
  (= (number-of-free-squares position) 1))

(define (number-of-free-squares position)
  (accumulate + (every (lambda (l) (if (equal? l '_) 1 0)) position)))